...
`setMode(mode)` in `instance.js` is the single function that toggles Calendar vs. Tasks visibility. It uses a dedicated `.msc-mode-hidden` class (not the native `hidden` attribute) on `.msc-calendar-main` and `.msc-view-dropdown`, because both already carry an unconditional `display` value in `calendar.css` that a same-specificity `[hidden]` rule cannot reliably beat — see the CSS comment at `.msc-calendar-main.msc-mode-hidden, .msc-view-dropdown.msc-mode-hidden` for the exact specificity reasoning. `.msc-tasks-main` follows the inverse, existing `.msc-view-pane`/`.active` idiom (`display:none` by default, `.msc-mode-active` shows it). `.msc-summary-section`/`.msc-list-card` (Today's Priorities) use the plain `hidden` attribute safely, since neither carries its own conflicting `display` rule.

**Addendum (2026-07-23, same day) — icon/order polish:** user feedback against the Google Calendar reference screenshot led to three fixes, all still in `web-view/css/calendar.css` and `web-view/js/calendar/instance.js`: (1) toolbar order corrected — sidebar-toggle now precedes the identity block, matching the reference; (2) the settings icon redrawn from an unclear 8-spoke "sunburst" to a hex-nut silhouette; (3) the Month/Week/Day segmented control (`.msc-view-switcher`) replaced by a `.msc-view-dropdown` trigger + anchored listbox menu, reusing the exact same `.msc-view-btn`/`viewSwitcherBtns`/`renderActiveView()` logic — only the container markup and ARIA role changed. Full detail and live-verification results in the validation doc's Addendum under "Toolbar result". No backend/database change, no regression (5-member structural check and blank-click smoke test re-run clean).
