...
## 13. Follow-up — Task Created/Updated at timestamps (2026-07-23, later same day)

Full record: the "Follow-up — Task Created/Updated at timestamps" section appended to `validation/google-inspired-task-leave-popup-ui-check-2026-07-23.md`.

**What this was:** add exactly two read-only rows, "Task Created at:" / "Task Updated at:", to the single Task Details popup only — discovery-first, no assumptions. A prior discovery-report turn confirmed both fields already exist end-to-end in the backend/API (`MemberScheduleEvent.created_at`/`.updated_at`, UTC-aware, non-nullable, already returned by `MemberScheduleEventOut`) but were silently dropped by the frontend mapper — so this was implementable as a frontend-only change with zero backend/API/database/migration change.

**Files modified this pass:**

- `web-view/js/calendar/core.js` — `apiItemToFrontend()` now passes `created_at`/`updated_at` through verbatim (previously dropped); new exported `formatTaskTimestamp(isoString)` converts a UTC ISO string to fixed-timezone `Asia/Colombo`, returns `YYYY-MM-DD HH:mm` (24-hour, no seconds) via `Intl.DateTimeFormat(...).formatToParts()` (locale-independent, deterministic), returns `'Not available'` for null/undefined/empty/invalid input. `frontendToApiPayload()` unchanged — these fields can never be sent back on Create/Update.
- `web-view/js/calendar/instance.js` — two new `<p class="msc-view-created-at">`/`<p class="msc-view-updated-at">` rows added to the existing Task Details markup (immediately after Notes, no new CSS needed — inherits the same unstyled-`<p>` treatment as Date/Time/Category/Priority/Notes); `viewItem()` sets both from `it.created_at`/`it.updated_at` via `formatTaskTimestamp()`. No other popup (Create/Edit Task, Leave Detail, chips, Task-list, Schedule Summary) was touched.

**Also this session (separate commit, prerequisite to the above):** the pre-existing uncommitted Full-Day Leave "blocks Create dialog" fix (already validated and approved from a prior session) was isolated and committed on its own — `Prevent Create dialog on full-day-leave dates` — before any timestamp work began, per explicit instruction not to combine or discard it.

**Live verification:** disposable `paraparan` records (`2099-12-28`/`2099-12-29`, `source_scope: dashboard_testing`) created/edited/dragged/resized against the real local FastAPI backend and real Neon database via `curl`, proving `created_at` stays constant and `updated_at` changes on every edit path (title, drag, resize) — all soft-deleted immediately after. `formatTaskTimestamp` verified in Node against the two required examples (`2026-07-23T08:35:00+00:00` → `2026-07-23 14:05`; `2026-07-23T11:12:00+00:00` → `2026-07-23 16:42`) plus null/invalid/day-rollover edge cases. `node --check` clean on both files; all 41 `core.js` imports (including the new `formatTaskTimestamp`) resolve; both new DOM-hook classes appear exactly once in markup and once in `querySelector`; all touched frontend assets returned HTTP 200 from a local static server. No Playwright/browser click-through was performed this pass (see validation doc §M for the substitute verification method used instead).

**Backend/API/database/migrations:** NONE. `git diff --stat -- backend/ database/ database/migrations/` empty for this pass.

**Commit:** see the repo's commit log for the exact hash at push time (commit message: "Show Task created and updated timestamps").
