...
A separate, explicitly-approved requirement is needed to: (1) define the real Issue-System backend endpoint(s) for fetch/assign/solving-status-update, (2) wire a real adapter implementation in place of `createProductionIssuesAdapter()`, and (3) have the backend independently enforce Admin authority for the assign action. Until then, this tab will always show the empty state in production, by design.
