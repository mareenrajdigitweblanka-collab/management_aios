...
>
> **Correction (2026-08-06, same-day, round 3):** §7's state-clearing table originally specified that a 401/authorization failure and leaving the dedicated tab should each only partially clear workspace state (both explicitly "keeps"/"remain valid"-worded to preserve the selected employee, and for the 401 row, the loaded history too). Implementation against that wording was flagged as a deviation from the intended approved behavior and corrected: **both a 401/authorization failure and leaving the dedicated tab now fully reset the workspace** — employee selection, loaded history, edit/draft state, and both filters — identically to a genuine token change, via one central `resetWorkspaceState()` function (replacing the prior `clearWorkspaceState()` name and its narrower 401/tab-leave paths). §7's table below reflects the corrected rule. See `validation/calendar-review-summaries-dedicated-tab-implementation-check-2026-08-06.md`'s "Correction — 2026-08-06 (state-clearing round 2)" section for the full implementation-level detail.
...
## 7. State-clearing design (Phase 8) — corrected round 3, 2026-08-06

Every trigger below routes through one central, reusable `resetWorkspaceState()` (`web-view/js/review-summaries.js`) — never a second, parallel reset path. It clears: the selected employee (UUID + display state); loaded history; expanded/full-summary card state (destroyed automatically, since cards are always rebuilt from scratch on every render, never patched in place); edit state; the unsaved summary draft and meeting-date draft; the reviewer filter (reset to `''`/"All reviewers"); both date filters; and any pending staff-search request (aborted). It always increments `state.historyRequestId`, invalidating any response still in flight from before the reset. It never touches the stored Calendar token itself — token clearing remains exclusively `handleUnauthorizedResponse()`'s job (calendar/auth.js), invoked only on a genuine 401.

| Trigger | Cleared | Mechanism |
|---|---|---|
| Employee change (including a fresh selection) | Everything `resetWorkspaceState()` clears (above), before the new employee is set | `selectStaff()` calls `resetWorkspaceState()` first, then sets `state.selectedStaff` |
| Reviewer-filter change | Stale edit/detail state; re-fetch | Sets `state.reviewerFilter`, exits edit mode, re-fetches; `historyRequestId` increments as part of the re-fetch |
| Date-filter change | Stale edit/detail state; re-fetch | Same pattern as the reviewer-filter change |
| Token change (a new/different member successfully authorizes) | **Full reset** — identity, history, edit/draft state, employee selection, reviewer/date filters | `CALENDAR_AUTH_CHANGED_EVENT` listener (§2.6) → `resetWorkspaceState()` → re-render from UNAUTHORIZED or AUTHORIZED-no-employee under the new identity; does not auto-reload the previous employee |
| Authorization failure (401 mid-session) or invalid-token startup | **Full reset** — identical to a genuine token change: employee selection, history, edit/draft state, reviewer/date filters | Same `CALENDAR_AUTH_CHANGED_EVENT` listener and the same `resetWorkspaceState()` call — a 401 and a genuine token change are treated identically, since `handleUnauthorizedResponse()` clears the stored token before dispatching the event either way |
| Leaving the dedicated tab | **Full reset** — identical to the two rows above: employee selection, history, edit/draft state, reviewer/date filters | `navigation.js:34`'s `msc:close-toolbar-popovers` event (fired on every panel switch, including leaving `#tab-review-summaries`) calls `resetWorkspaceState()` — repurposed from its old REQ-CAL-REV-001 role as a cross-tab "member changed" proxy (§2.6); the stored Calendar token is untouched, since ordinary navigation is not an authorization event |
| Returning to the tab | No employee selected, no history, reviewer filter at "All reviewers", date filters empty, zero history requests until an employee is selected | Direct consequence of the full reset already having run when the tab was left |
| 404 / owner-only mutation denial | Nothing — the stored token is explicitly NOT cleared, and this is never treated as an authentication failure | Only `res.status === 401` inside `reviewSummariesApiRequest()` calls `handleUnauthorizedResponse()`; a 404 (including the non-disclosing cross-reviewer/owner-only denial) falls through to the generic error-mapping path untouched |
...
| Stale in-flight response | discarded, never rendered — including one that resolves only after a 401/tab-leave/token-change reset has already fired | Every fetch carries the `historyRequestId` value current at fire time; on resolution, compare to `state.historyRequestId` — mismatch means a reset or newer request has since superseded it, so the response is dropped |

**Invariants required (corrected):** no history from a previous employee, a previous identity, or a previous tab visit remains visible after any reset; no edit/draft state leaks across any reset; no employee selection is ever restored across a 401, a token change, or leaving the tab; stale requests cannot overwrite current state under any trigger; a 404 or owner-only mutation denial never clears the token or is treated as an authentication failure. All are satisfied by the single `resetWorkspaceState()` function above, reusing existing patterns already present in `review-summaries.js` (`historyRequestId`, `CALENDAR_AUTH_CHANGED_EVENT`) rather than introducing new state-management primitives.
