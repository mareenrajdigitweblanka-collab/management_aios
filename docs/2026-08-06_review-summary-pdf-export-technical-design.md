...
>
> **Correction (2026-08-06, same-day, round 4):** Documentation-only — `pyproject.toml` and `backend/requirements.txt` were not modified producing this correction; no dependency was installed. Rounds 1-3 referred to `backend/requirements.txt` as *the* Gate B pin target without first confirming, from repository evidence, which file Vercel actually reads — this assumption is corrected. Repository inspection (`pyproject.toml`, `backend/requirements.txt`, root `requirements.txt`, `validation/member-schedule-vercel-function-crash-check-2026-07-10.md`) confirms `pyproject.toml` is the canonical production/Vercel dependency source (a prior production crash proved Vercel does not read `backend/requirements.txt` via the root `-r` reference; the fix, already deployed, added `[project.dependencies]` directly to `pyproject.toml`), while `backend/requirements.txt` remains actively required for local dev/test parity (it supplies `httpx`, a test-only dependency `pyproject.toml` deliberately omits, and is what local `pip install -r requirements.txt` reads) — not stale, not reference-only. §5.6 gains a new "Production dependency-source evidence" subsection and a restated "Final dependency-pin rule" (dual-file: `pyproject.toml` is production-authoritative, `backend/requirements.txt` is required for local parity, both must carry an identical exact pin, Gate B's test suite must assert they match). §12's dependency-file row is split into two rows (`pyproject.toml`, `backend/requirements.txt`). The 4/53/12/69 validation-plan count is unchanged — the dual-file version-drift check is folded into Gate B's existing 53-check category, not an additional numbered test. See the companion validation report's own round-4 correction note and `validation/review-summary-pdf-reportlab-preflight-2026-08-06.md`'s own wording correction for the full traceability update.
...
- Neither `pyproject.toml` nor `backend/requirements.txt` was modified by Gate A — this document is the only place `reportlab==5.0.0` is currently recorded. **Gate B applies this exact pin to `pyproject.toml` (the confirmed canonical production/Vercel dependency source — see "Final dependency-pin rule" and "Production dependency-source evidence" below) for the first time, and to `backend/requirements.txt` (actively required for local dev/test parity — evidenced below) in the same change, so the two never disagree.**
...
- Modifying `pyproject.toml` or `backend/requirements.txt` — the pin is Gate B's job, applied to both files only once Gate A has produced a verified version.
...
- Add **that exact verified version** as an exact pin — `reportlab==5.0.0` — to `pyproject.toml`'s `[project.dependencies]` array, using the same declaration style already used there (e.g. `"reportlab==5.0.0"` alongside the existing `"openpyxl>=3.1"`-style entries), since `pyproject.toml` is the confirmed canonical production/Vercel dependency source (see "Production dependency-source evidence" below). Add the identical exact pin to `backend/requirements.txt` in the same change, since repository evidence shows that file is actively required for local dev/test parity, not stale or reference-only (see below) — never a guessed or illustrative version string in either file. See "Final dependency-pin rule" below for the full dual-file rule and why a range/compatible-release specifier is never the approved implementation form.
...
### Production dependency-source evidence (new, round 4 — 2026-08-06, corrects an unstated assumption)

Rounds 1-3 referred to `backend/requirements.txt` as *the* Gate B pin target without first confirming, from repository evidence, which file Vercel actually reads. This round corrects that gap. Confirmed by direct inspection of `pyproject.toml`, `backend/requirements.txt`, root `requirements.txt`, and `validation/member-schedule-vercel-function-crash-check-2026-07-10.md` (a prior, unrelated production incident on this same backend):

- **`pyproject.toml` is the confirmed canonical production/Vercel dependency source.** Its own `[project.dependencies]` block carries a header comment recording exactly why: on 2026-07-10, production crashed with `ModuleNotFoundError: No module named 'fastapi'` because Vercel's Python build did **not** install packages via root `requirements.txt`'s `-r backend/requirements.txt` reference — Vercel's dependency detection does not follow that indirection. The fix (`validation/member-schedule-vercel-function-crash-check-2026-07-10.md` §8) added `[project.dependencies]` directly to `pyproject.toml`, mirrored from `backend/requirements.txt`, specifically so Vercel would install the required packages regardless of the `-r` reference's fate. This is the currently-deployed fix — `pyproject.toml` is the file Vercel actually reads.
- **`backend/requirements.txt` is not stale or reference-only — it is actively required for local development and tests.** Evidence: (a) root `requirements.txt` is `-r backend/requirements.txt`, kept per `pyproject.toml`'s own comment "for local `pip install -r requirements.txt` parity" — i.e. it is the file a local developer's `pip install` actually reads; (b) it declares `httpx>=0.27` under an explicit "Test-only" comment, required by `fastapi.testclient.TestClient` for `backend/tests/test_calendar_auth.py` and `test_calendar_mutation_authorization.py` — a dependency `pyproject.toml` deliberately does **not** carry (correctly, since it is not a production runtime import). `backend/requirements.txt` therefore has a real, evidenced, ongoing role distinct from `pyproject.toml`'s.
- **No automated synchronization mechanism exists between the two files.** No CI configuration (`.github/workflows/` or equivalent) was found anywhere in this repository, and no sync/lint script referencing both files was found. The two files' currently-shared runtime dependencies (`fastapi`, `uvicorn[standard]`, `sqlalchemy`, `psycopg[binary]`, `pydantic`, `python-dotenv`, `openpyxl`) happen to match today, at identical version constraints, but nothing in this repository enforces that they stay in sync going forward — this was a real, previously-unstated risk this round makes explicit rather than continuing to leave implicit.

### Final dependency-pin rule (updated, round 4 — dual-file rule, canonical source identified)
...
4. Gate A has now run (2026-08-06) and verified **`reportlab==5.0.0`** on local Python 3.14.4 — see "Gate A result" above. This is the only version Gate B is authorized to pin, unless a future re-run of Gate A verifies a different one.
5. **Gate B must place the exact verified pin in this repository's canonical production dependency source — `pyproject.toml`** (per "Production dependency-source evidence" above), added to its existing `[project.dependencies]` array using the array's own established declaration style (a plain quoted `"package==version"`/`"package>=version"` string per line, matching the existing `"openpyxl>=3.1"` entry's form).
6. **`backend/requirements.txt` must also be updated in the same Gate B change**, because repository evidence (above) proves it is actively required for local tests and backend execution — this is not a discretionary mirror, it is required by the same standard rule 5 of the original task instruction ("`backend/requirements.txt` may also be updated during Gate B only when repository evidence proves it is actively required for local tests or backend execution") being satisfied here.
7. **Dual-file synchronization rule, since both files are required**: both `pyproject.toml` and `backend/requirements.txt` must carry the exact same `reportlab==5.0.0` pin — never two different versions, never a range in one and an exact pin in the other. Gate B's test suite (§16's existing 53-check Gate B category — no new numbered test is added by this rule, this is a required detail of that existing category's dependency-pin coverage) must include a test that reads both files and asserts their `reportlab` version constraints are identical, so any future drift between them is caught automatically rather than discovered in production. Documentation (this section, and `validation/review-summary-pdf-export-design-check-2026-08-06.md`) identifies `pyproject.toml` as the production-authoritative file and `backend/requirements.txt` as the local-dev/test-parity file — two files, one explained, evidenced relationship, not two unexplained truths.
8. An unbounded or open-ended dependency specifier is never the approved implementation form in either file — none of `reportlab`, `reportlab>=4`, or a bare compatible-release specifier (e.g. `~=4.x`) may be what actually lands in `pyproject.toml` or `backend/requirements.txt`. A syntactically valid compatible-release expression may be used *only* to explain pin syntax in prose (as this rule itself does, describing the *shape* `package==version` takes), never as a stand-in for the real, Gate-A-verified exact pin.
9. Gate C remains responsible for proving what Gate A and Gate B cannot: that Vercel actually selects a compatible Python interpreter for the pinned `reportlab==5.0.0` to run under, that the package installs in the real preview build, that the function starts successfully, and that the export endpoint operates correctly end-to-end (§ "Gate C — Vercel preview validation" above) — pinning the dependency in the correct file (this section) is necessary but not sufficient for a production-compatibility claim.

This gate is a precondition for implementation sign-off, not something this design session can complete itself — no dependency was installed, no synthetic PDF was generated, and no Vercel preview was deployed while producing this design or this correction (§14 confirms). Neither `pyproject.toml` nor `backend/requirements.txt` was modified by this correction, per this task's explicit instruction.
...
| `pyproject.toml` | **In Gate B only** (§5.6), add `"reportlab==5.0.0"` to the existing `[project.dependencies]` array, matching that array's own declaration style — this is the confirmed canonical production/Vercel dependency source (§5.6 "Production dependency-source evidence"), not `backend/requirements.txt` alone |
| `backend/requirements.txt` | **In Gate B only** (§5.6), add the identical exact-pinned line `reportlab==5.0.0` in the same change as the `pyproject.toml` edit — evidenced as actively required for local dev/test parity (§5.6), not a discretionary mirror; both files' `reportlab` pin must match exactly (dual-file synchronization rule, §5.6) |
| Both files above | Version Gate A's preflight verified on 2026-08-06 (`validation/review-summary-pdf-reportlab-preflight-2026-08-06.md`), never a range or illustrative version string. Pure-Python PDF library; **neither file installed nor modified by Gate A or any prior correction round**, per this task's explicit instruction; production compatibility remains unverified until Gate C passes |
...
**Status: Gate A PASSED — `reportlab==5.0.0` verified (2026-08-06, `validation/review-summary-pdf-reportlab-preflight-2026-08-06.md`) — Gate B authorized, not yet started; canonical pin target confirmed as `pyproject.toml` (updated, round 4).**
...
54. A clean virtual environment (outside tracked repository assets) installs the candidate `reportlab` version selected for preflight (§5.6 Gate A) — **executed and PASSED 2026-08-06 with `reportlab==5.0.0`**, `validation/review-summary-pdf-reportlab-preflight-2026-08-06.md` — this check has no bearing on `pyproject.toml` or `backend/requirements.txt`, neither of which is touched until Gate B.
...
- **Technical review**: required for the new route, the `_build_review_summary_query` extraction, the route-declaration-order safeguard (§5.1), the empty-result 404 design (§5.4), and the `reportlab` dependency choice — **Gate A (pre-implementation preflight) has now passed (2026-08-06, `validation/review-summary-pdf-reportlab-preflight-2026-08-06.md`), verifying `reportlab==5.0.0`; the canonical Gate B pin target is confirmed as `pyproject.toml`, with `backend/requirements.txt` updated in the same change (§5.6 "Production dependency-source evidence," updated round 4); Gate C (Vercel preview) remains the one open follow-up and must pass before any production-compatibility claim is made.**
