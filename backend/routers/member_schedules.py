...
from datetime import date as date_type, datetime, timedelta, time as time_type, timezone
...
def classify_schedule_category(
    ...
    - requested 'Unscheduled Task' always returns 'Unscheduled Task'
      (a user may intentionally opt out early; nothing overrides that).
    - start_time is None (untimed task) returns the requested category
      unchanged — no planned start time exists to compare against, so an
      untimed task is never automatically treated as late.
    - otherwise, event_date + start_time is composed as Asia/Colombo local
      time, converted to a UTC instant, and compared against created_at
      (already a UTC instant, generated by the caller before this is
      invoked): created_at <= planned_start -> requested category is
      honored; created_at > planned_start -> forced to 'Unscheduled Task'
      regardless of what was requested. The comparison is inclusive
      (<=) so a task created at exactly its planned start time may still
      be Scheduled Task.
    """
    ...
    planned_start_local = datetime.combine(event_date, start_time, tzinfo=_COLOMBO)
    planned_start_instant = planned_start_local.astimezone(timezone.utc)

    if created_at <= planned_start_instant:
        return requested_category
